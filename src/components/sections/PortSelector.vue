...
watch([() => props.availableSerialPorts, () => props.availableAdbDevices], ([ports, devices]) => {
  const selected = selectedPort.value;
  if (selected?.type === 'serial') {
    if (!ports.includes(selected.path)) {
      selectedPort.value = null;
    }
  } else if (selected?.type === 'adb') {
    if (!devices.find(({ identifier }) => identifier == selected.identifier)) {
      selectedPort.value = null;
    }
  }
});
